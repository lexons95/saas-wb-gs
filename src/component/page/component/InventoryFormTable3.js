...
import { Table, Input, Button, Popconfirm, Form, InputNumber, Modal, Tooltip, Switch, Checkbox, Divider, Space } from 'antd';
...
const InventoryFormTable3 = (props) => {
  const { productId, productVariants, setProductVariants, onCancel, refetchList } = props;
  ...
  const { data: inventoryResult, loading: loadingInventory, error, refetch: refetchInventory } = useInventoryQuery({
    filter: {
      filter: {
        productId: productId
      },
  ...
  const [ updateInventory, updateInventoryResult ] = useMutation(BULK_UPDATE_INVENTORY_QUERY,{
    onCompleted: (result) => {
      ...
      showMessage({type: 'success', message: 'Success: Inventory Updated'});
    }
  })
  ...
  useEffect(()=>{
    if (inventoryResult && inventoryResult.inventory) {
      ...
      inventoryResult.inventory.map((anInventory,index)=>{
  ...
  const getColumns = () => {
    ...
    let defaultColumns = [
      {
        title: 'Price' + (configCache && configCache.currencyUnit ? ` (${configCache.currencyUnit})` : ''),
        dataIndex: 'price',
        editable: true,
        width: 110,
        //fixed: 'left',
        ...
        render: (text, record) => {
          return (
              <div style={{width: '100%', textAlign: 'center', cursor: 'pointer'}}>
                ...
                {/* <Checkbox checked={record.published} onChange={(e)=>{handleUpdatePublished(record, e.target.checked, e)}}/> */}
              </div>
          )
        } 
      },
    ...
    // return result.concat(defaultColumns).concat(actionColumn);
    return indexColumn.concat(defaultColumns).concat(result).concat(actionColumn);
  }
  ...
  const handleUpdateInventory = () => {
    if (productId) {
      ...
      if (inventoryResult && inventoryResult.inventory) {
        inventoryResult.inventory.map((anInventory)=>{
  ...
  let isLoading = loadingInventory;
    ...
    <div className="productForm-tab-container">
      <div className="productForm-tab-content">
        <div className="productForm-productInfo-container">
          <div className="inventoryFormTable3-container">
      ...
      <div className="productForm-tab-footer">
        <div className="productForm-tab-inventory-footer">
          <Button
          ...
          <Space>
            ...
            <Button key={'submit'} type="primary" onClick={handleUpdateInventory} disabled={false}>
              Save
            </Button>
          </Space>
        </div>
...
export default InventoryFormTable3;